...
// https://blog.csdn.net/tyBaoErGe/article/details/50375802
...
func Spy(url string) {
	...
	if e != nil {
		fmt.Errorf("error in Get request %s: %s", url, e)
		return
	}
...
func main() {
	...
	for url := range urlChannel {
		fmt.Println("\nroutines num=", runtime.NumGoroutine(), "chan len=", len(urlChannel))
		go Spy(url)
	}
	fmt.Println("a")
}
